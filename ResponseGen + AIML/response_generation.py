...
with open(f"{path_to_respgen}/Tokens.txt", 'r') as file:
    js_string = file.read()
...
embeddings_index = {}
f = open('bot/glove.42B.300d.txt',encoding="utf8")
for line in f:
    values = line.split()
    word = values[0]
...
# Loading LDA Model
lda_model = models.ldamodel.LdaModel.load(f'{path_to_respgen}/LDA.model', mmap = 'r')
...
from gensim import corpora
topic_dict = corpora.Dictionary.load(fname = f'{path_to_respgen}/topic_dict.dict', mmap = 'r')
try :
...
checkpoint_dir = f'{path_to_respgen}/training_checkpoints'
